...
- APScheduler 运行在 FastAPI asyncio 事件循环中，负责所有轻量定时任务；无需 celery-beat 容器。已注册的 job：`strategy_probe`（探测完成→触发 LLM 审查，2 min）/ `strategy_collection`（全量完成→建切片；切片 Stage2 全完→跑 coverage_check 推进 ready，2 min）/ `news_task_watchdog`（新闻任务超时回收，probe + collect，5 min）/ `agent_timeout_reset`（agent 超时回收，5 min）/ `crawl_nbs`/`crawl_cnnic`/`crawl_govsite`（KB 爬虫，月度/周度）。详见 `strategies/CLAUDE.md § 定时任务`
