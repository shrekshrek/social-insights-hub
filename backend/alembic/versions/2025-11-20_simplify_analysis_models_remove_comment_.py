...
def upgrade() -> None:
    ...
    op.add_column('post_analysis', sa.Column('post_deep_result', sa.JSON(), nullable=True, comment='帖子深度分析结果：实体、观点、摘要'))
    op.add_column('post_analysis', sa.Column('comment_deep_result', sa.JSON(), nullable=True, comment='评论深度分析聚合结果：从评论中提取的实体和观点'))
    ...
    bind.execute(sa.text("DELETE FROM task_analysis_results WHERE analysis_type = 'screening_comments'"))
...
def downgrade() -> None:
    ...
    op.add_column('post_analysis', sa.Column('depth_analysis_result', sa.JSON(), nullable=True))
