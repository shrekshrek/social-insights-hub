...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "crawler_type",
            sa.Enum("search", "detail", "creator", "homefeed", name="crawlertype"),
            nullable=False,
            comment="爬取模式",
        ),
        sa.Column(
        ...
        sa.Column(
            "checkpoint_id", sa.String(length=255), nullable=True, comment="检查点ID"
        ),
        sa.Column(
            "checkpoint_data",
            postgresql.JSON(astext_type=sa.Text()),
    ...
    op.create_index(
        op.f("ix_crawler_tasks_platform"), "crawler_tasks", ["platform"], unique=False
    )
    op.create_index(
        op.f("ix_crawler_tasks_status"), "crawler_tasks", ["status"], unique=False
    )
    op.create_index(
