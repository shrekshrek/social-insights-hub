"""add unique constraint (monitor_id, name) to social_slices and news_slices
...
问题背景：
  ...
  导致同名切片被创建多份，且 pipeline 未正确启动。

修复方案：
  在 DB 层对 social_slices 和 news_slices 均添加 (monitor_id, name) 唯一约束，
  并发时第 2~4 个 INSERT 抛 IntegrityError，_create_auto_slices 捕获后幂等跳过。
"""
