...
from src.analysis.celery_tasks.llm_utils import (
    invoke_chain_with_stats_sync,
)
from src.analysis.celery_tasks.aggregation.utils import (
    calculate_score,
...
def aggregate_opinions(
    ...
    if raw_category_count > 5:
        try:
            ...
            cat_response, cat_stats = invoke_chain_with_stats_sync(
                cat_norm_chain, {"categories": formatted_cats}, llm_type
            )
            ...
            response_text = (
                cat_response.content
                if hasattr(cat_response, "content")
                else str(cat_response)
            )
    ...
    grouped_raw_opinions: Dict[str, Dict[str, set]] = defaultdict(
        lambda: defaultdict(set)
    )
    # 结构: {group_key: {raw_term: heat}}
    grouped_term_heat: Dict[str, Dict[str, float]] = defaultdict(
        ...
        for source_type, source in sources:
            ...
            if isinstance(opinions, list):
                for op_obj in opinions:
                    ...
                    # 计算观点权重：原文使用 post_impact，评论使用 support_score
                    if source_type == "post":
                        opinion_weight = post_impact
                    else:
                        # 评论观点：使用 support_score 计算权重
                        support_score = op_obj.get("support_score", 0)
                        opinion_weight = calculate_comment_weight(
                            support_score, post_impact
                        )
                    ...
                    for term in terms:
                        if term and isinstance(term, str) and len(term) > 1:
                            ...
                            # 追踪来源类型
                            grouped_term_sources[group_key][term][source_type].add(
                                post_id
                            )

                            # 热度累加 (Per Post Per Term 去重)
                            # 原文使用 post_impact，评论使用 support_score 权重
                            if (
                                post_id
                                and term_key not in post_contribution_map[post_id]
                            ):
                                grouped_term_heat[group_key][term] += opinion_weight
    ...
    for group_key, raw_map in grouped_raw_opinions.items():
        ...
        if len(top_k_map) <= 5:
            for term, post_ids in top_k_map.items():
                ...
                source_dist = {
                    "post": round(post_count / total_sources, 2)
                    if total_sources > 0
                    else 0,
                    "comment": round(comment_count / total_sources, 2)
                    if total_sources > 0
                    else 0,
                }
            ...
            for term in originals:
                if term in top_k_map:
                    ...
                    merged_originals.append(
                        {"text": term, "count": len(top_k_map[term])}
                    )
                    # 合并来源类型
                    term_sources = grouped_term_sources[group_key][term]
    ...
    if token_stats["summary"].get("total_duration_seconds", 0) > 0:
        token_stats["summary"]["total_api_duration_seconds"] = token_stats["summary"][
