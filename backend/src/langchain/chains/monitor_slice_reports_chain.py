...
5. **主动寻找反直觉信号 (Counter-Intuitive)**：不要只报告热门现象，要主动检查以下模式（有发现才写，没有就不写）：
   ...
   - **推广与口碑背离**：某话题或实体的 `organic_sentiment` 为负而 `promo_sentiment` 为正，差值 ≥ 0.3，说明推广内容正在��盖真实用户体验。
...
**本节仅在数据中确实存在以下任一模式时输出，否则省略整节。不要为了输出此节而牵强解读。**
...
3. **数据不对称风控 [重要]**：
   ...
   - **采集顺序偏差**：系统按“首见原则”去重，同时提及本品和竞品的帖子会优先归入本品任务，导致竞品的正面口碑可能被系统性低估。做竞品对比时如发现竞品数据量明显偏少，需注明”竞品真实口碑可能优于当前数据所呈现”。
