...
## 任务背景
输入是一组用户在社交媒体上的真实评论片段（已按类别分组，如"价格"、"服务"等）。
你需要将这些碎片化、口语化的表达，归纳为专业、简洁的**标准观点**。

## 任务要求
1. **语义合并**：将表达相同含义的观点合并。
   - 例："太贵了"、"买不起"、"死贵" -> 归纳为 "定价过高"
   - 例："客服半天不回"、"找不到人工" -> 归纳为 "客服响应慢"
2. **提取标准名 (Label)**：
   - 为每个聚类生成一个简练、专业、具有描述性的**陈述句**或**短语**。
   - 必须客观、准确，**忠实反映原始含义**，保留情感倾向（不要把负面洗成中性）。
3. **保持原意**：标准观点必须忠实反映原始观点的含义，不要过度推断。
3. **覆盖率**：输入列表中的所有有效观点都应该被归入某个聚类中。
4. **去噪**：对于完全无意义的语气词或无关内容，可以丢弃。
