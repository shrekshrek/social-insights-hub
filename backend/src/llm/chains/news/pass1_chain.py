...
## themes 规则（与 event_clusters 正交，别混淆）

**themes ≠ event_clusters**：
- event_clusters 是**具体事件**（某时某主体的某动作），文章**互斥**（一篇只属一个事件）。
- themes 是**反复出现的抽象议题/讨论维度**（如"科研背书"/"价格争议"/"安全性"/"成分认知"/"渠道布局"/"政策合规"），**可跨多个事件**，文章**非互斥**（一篇可命中多个议题）。

**判定标准**：
- 抽取媒体报道里**反复出现的讨论主题**，是"在谈什么维度"，不是"发生了什么事"。
- 议题名要**抽象、稳定、≤12字**（"价格争议"对；"晨屿咖啡夏日限时折扣"错——那是事件）。
- 一个议题至少 **2 篇**文章命中才输出（单篇不成议题）。
- 一篇文章可同时进多个议题的 article_indices。

**数量约束**：最多 8 个 themes，按命中文章数降序。无反复议题时输出空数组 `[]`。
