...
4. **上下文意识 (Context Awareness) [重要]**：
   - 本报告基于**用户选定的特定任务/关键词**生成的“本次切片范围内的洞察”，而非全网绝对全景。
...
- **头部玩家画像**：用一句话精准概括 Top3 品牌的核心心智（例如：3M——靠品牌光环躺赢；戴森——高端技术标杆但溢价被质疑；元气森林——健康概念贴标成瘾）。

### 3. 关键趋势与心智 (Key Trends)
- 提炼 2-3 个正在发生的行业趋势（如”消费降级导致平替兴起”、”服务体验成为新战场”）。

## 声量口径说明
数据中含 `organic_heat`（自然声量，low_spam 原文的加权热度）与 `promo_heat`（推广声量，high_spam 原文的加权热度）两个维度。分析品牌声量时：
...
5. **主动寻找反直觉信号 (Counter-Intuitive)**：不要只报告热门现象，要主动检查以下模式（有发现才写，没有就不写）：
   - **高极化低热度议题**：`controversies` 中 `controversy_depth ≥ 0.4`（两极均衡度，接近 0.5 说明正负意见几乎对半）且 `polar_total >= 10`（正负极性提及之和，确保极性样本可信），但 `heat` 不突出的话题，是尚未引爆的隐性矛盾。
   - **推广与口碑背离**：某话题或实体的 `organic_sentiment` 为负而 `promo_sentiment` 为正，差值 ≥ 0.3，说明推广内容正在掩盖真实用户体验。
...
### 5. 反直觉信号 (Counter-Intuitive Signals)
- **隐性撕裂议题**：`controversies` 中 `controversy_depth ≥ 0.4` 且 `polar_total >= 10`（正负极性提及之和，确保极性样本可信）但 `heat` 不突出的话题，正负意见几乎对半，是尚未引爆的隐性矛盾点。
- **推广与口碑背离**：某话题或实体 `organic_sentiment` 为负而 `promo_sentiment` 为正，差值 ≥ 0.3，说明推广在掩盖真实口碑。

## 声量与情感口径说明
话题数据含两套情感维度：
- `sentiment`：综合情感（所有原文加权均值）。
- `organic_sentiment`：自然用户的真实情感（low_spam 原文）。
- `promo_sentiment`：推广内容的情感（high_spam 原文）。
...
3. **数据不对称风控 [重要]**：
   ...
   - **采集顺序偏差**：系统按“首见原则”去重，同时提及本品和竞品的原文会优先归入本品任务，导致竞品的正面口碑可能被系统性低估。做竞品对比时如发现竞品数据量明显偏少，需注明”竞品真实口碑可能优于当前数据所呈现”。
...
def create_monitor_slice_landscape_report_chain() -> Runnable:
    return _chain(LANDSCAPE_SYSTEM, LANDSCAPE_USER)


def create_monitor_slice_topic_report_chain() -> Runnable:
    return _chain(TOPIC_SYSTEM, TOPIC_USER)


def create_monitor_slice_focus_report_chain() -> Runnable:
