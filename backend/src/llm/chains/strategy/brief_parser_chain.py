...
- `channel_brief`: 本渠道可切入的 brief 诉求概述（1-2句）。允许与其他渠道描述的诉求重叠——具体承接由下游 research_design 按维度决定。务必覆盖原 brief 中属于本渠道能力范围的所有诉求（包括竞品对比、品牌认知等跨渠道诉求），不要因"另一渠道也能做"而省略。**channel_brief 必须具体到可检索的研究对象，不用"竞品""品牌""市场"等泛称**：默认点出研究主体(subject)与竞品(competitors)的具体名称（如"围绕 <主体> 及竞品 <竞品A>、<竞品B>…"）；**唯一例外**——本渠道做的是品类/市场层面的分析（研究对象是整个市场、而非具体品牌，如行业研究）时，锚定到具体品类/市场（如"<品类>市场"），不收窄成单一品牌。泛称会让下游关键词退化成无锚泛词。（与"评估维度"区分：评估维度仍只放 `(评估维度：…)` 标注。）
