...
- **knowledge_base**（市场知识库）：平台内置以宏观数据为主（CNNIC 互联网统计报告、国家统计局月度指标、gov.cn 数字经济/平台经济政策文件），细分品类/品牌内容需用户自行上传研报或历史资料补充；仅适合需要宏观背景佐证（行业规模、用户总量、政策环境）的场景，不适合细分品类竞品分析或消费者口碑研究
