...
（默认 baidu + sogou + duckduckgo，可选 wechat_mp；单渠道上限 20 条）
...
- **文章卡片**：多渠道搜索（默认 baidu + sogou + duckduckgo，可选 wechat_mp）合并去重后的结果，
...
- **pass**：标题与摘要反映的话题与该维度研究问题的核心关注点有明显关联，
  且 tier1+tier2 合计占比 ≥ 20%（信号可信）
- **fail**：出现以下任一情形
  - 结果严重跑题（如搜"新能源汽车电池"回的全是手机锂电池、电动自行车电池）
  - 主体歧义（如搜"蔚来"混着车企与无关同名项目）
  - 通篇广告/榜单/导购（`xxx 十大排行榜`、`2026 最新报价`）
  - tier1+tier2 合计占比 < 20%（信号过弱，缺权威来源背书）
...
新闻搜索关键词技巧：
- 加限定词消歧义："蔚来" → "蔚来汽车" / "蔚来 ET7"
- 加领域词提升权威源命中："固态电池" → "固态电池 产业" / "动力电池 行业"
- 去掉过于宽泛的商品名，改用产业/政策/品牌术语
- 如果研究问题本身不适合新闻媒体回答（如消费者口碑/用户体验），
  可将 suggested_keyword 设为 null，表示建议直接移除该任务

**suggested_keyword 是提交给搜索引擎的单一查询字符串**（可含空格），
禁止用 `|` 拼接多个备选。
