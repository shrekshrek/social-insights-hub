...
### 1.5 受众不作为 data_plan 拆分依据

关键词搜索召回的是讨论某话题的帖子，作者画像不可控。**禁止用受众标签作维度名后缀**（如 `consumer_voice_<某受众群>`）——受众识别由切片/Insight 链从内容反推，data_plan 按 §1/§2 正常设计即可。
...
- **研究主题锚定优先于泛属性词**（所有维度通用原则）：涉及具体品牌/主体的关键词（主要是 competitive 和 consumer_voice 维度），**默认都采用「品牌/主体 + 研究主题/核心事件词」的形式**（如"3M 车衣"、"蔚来 固态电池"、"瑞幸 9.9退场"——分别对应品类锚/技术锚/事件锚三类），而非裸品牌名或"品牌 + 泛评价词"（如"怎么样/测评/体验/价格/吐槽"）。价值在于**让同维度所有品牌关键词结构对齐、召回数据落在同一话题范围内，保证横向对比公平**——尤其对广谱品牌（业务横跨多品类，如 3M、索尼、西门子），裸名会引入大量跨领域噪音；对专精品牌（业务聚焦单一品类），加锚略减召回量但不损精度，是保持横向一致性的可接受成本。不要让 LLM 逐品牌判断"是否广谱"，统一加锚即可。只有当研究主题本身无合适锚词（如目标就是观察品牌整体声量，无事件/品类细化），才退化为裸品牌名；泛评价词仅作最后兜底
- 品牌自发声维度（brand_voice）用品牌官方账号名、Campaign 名、发布会主题等，限于品牌主动传播内容
- 消费者维度（consumer_voice）是社媒 UGC 的主要入口。**横向对比与深挖分维度的强制规则**：若本研究设计同时包含 competitive 维度，consumer_voice 主品关键词**必须与 competitive 竞品关键词采用相同的主题锚**，保证横向对比结构对齐。**横向对比维度内每条 keyword 对应独立品牌（不同公司/独立 brand），主品 vs 竞品 keyword 数严格对等**：通常主品 1 条、每个独立竞品 1 条。**反例**：同品牌的不同产品线/系列（如「品牌X」+「品牌X 蓝钻」、「品牌X」+「品牌X 未来」）作多条 keyword 是伪扩充——同品牌多产品线属单品牌深挖，独立成 dim。**边界出口**：若 brief 实际独立竞品 <2 个，**直接不建 competitive 维度**（转主品深挖即可），不要凑伪横向——单一竞品的横向对比无意义，且会让大盘 SOV 输入不平衡。任何品牌若需要 RQ 深挖（事件/诉求/品牌概念等非共享主题锚方向），独立成**单品牌深挖维度**（dimension_name 带"深挖"标识），不与横向对比维度混在一起。模式：横向对比 = `[<品牌A> <共享主题锚>, <品牌B> <共享主题锚>, ...]`；单品牌深挖 = `[<品牌X> <特定事件/概念>]`
...
- 竞品维度（competitive）只放竞品品牌词，主品归 consumer_voice——同一品牌**不在两个维度的 keywords 中重复出现**。**所有竞品关键词默认都加研究主题/核心事件锚**，结构统一（例：研究车衣品类时 `["3M 车衣", "龙膜 车衣", "威固 车衣"]`）。不要因为某品牌"只做本品类"就单独省略锚词，统一结构更便于横向对比。**维度命名应只反映单一 dimension type**（dimension_name 与 RQ.dimension 一一对应）——跨 dimension type 的混合命名会诱导把主品+竞品塞进同一维度。**反例**：`产品口碑对比` / `品牌成分讨论` 这类"对比/综合"命名 → 必拆为 consumer_voice 主品维度 + competitive 竞品维度两条独立 dim，共享同一主题锚实现结构对等（不要为了"keyword 结构对齐"而合并）
...
- **横向对比维度组合（competitive + 配对的 consumer_voice 主品横向维度）必须同时加入品牌聚焦切片和大盘分析切片**：若 data_plan 含 competitive 维度，连同与之共享主题锚的 consumer_voice 主品横向维度（1 keyword 主品 + 主题锚）都要进入两个切片——品牌聚焦切片用作竞品质性对比，大盘分析切片用作 SOV 声量对比（**SOV 必须主品+竞品同框才完整**，只放竞品会让主品声量=0）
