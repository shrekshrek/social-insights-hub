...
- **同维度内关键词结构与语义双重一致**：由于本维度所有品牌关键词统一采用"品牌 + 主题锚"结构，字符结构自然对齐，召回数据也落在同一研究主题范围内——这两者一起保证横向对比可靠。禁止出现本维度部分关键词加主题锚、部分用裸名的混用情况（会让对比基准错位）。**且主题锚必须处于同一抽象层级**：要么全部是具体品类词（如"窗帘机器人"/"降噪耳机"/"车衣"），要么全部是事件词（如"涨价"/"618"），要么全部是场景词（如"租房"）；**禁止具体品类词与上位大类词混用**（如"窗帘机器人"与"智能家居"、"降噪耳机"与"消费电子"——上位词召回过宽，对比基准被拉偏）。自检：把所有关键词剥掉品牌名，剩余 token 应当语义相近、覆盖范围相当
- **同一维度的关键词必须属于同一赛道/品类**。如果 Brief 涉及多个不同赛道，必须拆成独立维度分别采集，不要混在同一组关键词中——混合搜索会导致数据噪音，实体提取和竞品对比无法正确归类。**赛道判定方法**：若两个品类**无法用同一个研究主题锚同时描述**（例如"窗帘机器人 租房"和"扫地机 租房"是两个完全不同的搜索意图，没有共用的下位锚词），即视为不同赛道，必须拆为独立维度（如 `competitive_curtain` + `competitive_vacuum`）。不要把"智能家居""消费电子""家电"这类大类词当作"赛道"来合并多个具体品类
...
### 3. 控制采集规模
每个关键词在每个平台的采集量约 40 条，分析耗时适中。方案必须精简：
