...
- 消费者维度（consumer_voice）是社媒 UGC 的主要入口：讨论主品时默认用「主品名 + 研究主题/事件词」（保持与竞品维度结构一致），其次才是「主品名 + 评价/体验/怎么样」；调研通用品类需求时用需求词（痛点/场景/决策），两类用法不要混在同一组 keywords
...
- **各平台搜索特点**（关键词格式需适配平台机制，但**"主题锚定优先"原则仍优先适用**——以下属性词仅在研究主题无合适锚词时作为兜底）：
  - 知乎：问题型关键词效果最好（"如何选择X""X怎么样"）；首选"品牌 + 研究主题/事件"，其次才用"品牌 + 怎么样"；注意"品牌名+怎么样"可能召回求职/雇主评价而非产品讨论
  - 微博：事件/热点驱动，首选"品牌 + 研究主题/事件词"（如"品牌 新品发布"、"品牌 涨价"、"品牌 停产"、"品牌 召回"），其次才用"品牌 + 口碑类词（口碑/评价/怎么样）"
  - 小红书：体验测评导向，首选"品牌 + 研究主题/场景"，其次才用"品牌 + 测评/体验/推荐/避坑"；平台内容偏消费生活，研究主题需与平台用户讨论生态匹配
  - 抖音：首选"简短品牌名 + 研究主题/场景词"，其次用"品牌 + 评测词"；平台内容偏大众消费和娱乐，研究主题需有足够的视频化讨论基础
  - B站：内容深度优于抖音，首选"品牌 + 研究主题分析/对比"；专业话题可用行业分析型词（"行业趋势/技术对比"）
