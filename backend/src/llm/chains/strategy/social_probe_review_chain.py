...
- **采集指纹唯一性**：建议的 (suggested_keyword, platform) 元组**不得与本研究设计中已有任何 task 的 (keyword, platform) 元组完全相同**（输入会列出所有现存 task 的元组清单）——否则会创建完全重复的采集任务。若想让某 dim 召回类似内容，必须用**字面不同但语义相关**的词（如"X 冷萃版 体验" vs 已有"X 冷萃"）
...
- note 必须明确写出两条路径命中情况 + 量化数（如「subject_match=True（召回晨屿 8 次），话题路径相关源帖 4，deep_analyzed=12」），让审查者可追溯
