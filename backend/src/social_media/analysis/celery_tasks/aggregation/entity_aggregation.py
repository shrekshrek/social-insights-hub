...
def build_entity_name_mapping(
    ...
    logger.info(f"[实体归一化] 程序相似度: {raw_count} -> {after_similarity_count} 个 (合并了 {similarity_merged} 个)")
        ...
        try:
            ...
            if len(llm_input_entities) >= 5:
                ...
                if llm_mapping:
                    ...
                    logger.info(f"[名称映射] LLM 归一化后: {final_unique} 个唯一实体 (合并了 {llm_merged} 个)")
...
def _clean_entity_attributes_sync(
    ...
    all_fields = ["features", "issues", "expectations", "audience", "scenarios", "market_factors", "competitors"]
    ...
    for res in results:
        ...
        for cluster in clusters:
            ...
            for term, count in cluster_terms_stats.items():
                new_map[label]["original_terms"].append({
                    "text": term,
                    "count": count
                })
...
def aggregate_entities(
    ...
    if enable_llm_normalization and tags_mapping:
        ...
        logger.info(f"[实体聚合] {len(raw_entity_data)} 个原始实体 -> {before_filter} 个归一化 -> {len(entity_data)} 个 LLM 实体")
    else:
    ...
    for data in entity_data.values():
        ...
        def format_attr_items(data_dict, max_items: int | None = None):
            ...
            result = []
            # 按 count 排序
            sorted_items = sorted(data_dict.items(), key=lambda x: get_item_count(x[1]), reverse=True)
            ...
            for text, item in sorted_items:
                if isinstance(item, dict) and "post_ids" in item:
                    # 已经是新结构 (Top N 清洗过的)
                    sorted_terms = sorted(item["original_terms"], key=lambda x: x["count"], reverse=True)
                    attr_item = {
                        "text": text,
        ...
        entity_dict = {
            ...
            "market_factors": format_attr_items(data.get("market_factors", {}), attr_limit),
