...
- 4 维 (SpamDistribution): 实体、观点 — 高/低广告 × 原文/评论
- 2 维 (SpamCountBreakdown): IPA、竞品雷达、时间分布 — 高/低广告
...
def build_spam_distributions(
    ...
    for point in ipa_points:
        point["spam_distribution"] = _compute_spam_dist_2d(
            point.get("post_ids", []), spam_map
        )

    # 2 维: 竞品雷达
    for series in competitor_series:
        series["spam_distribution"] = _compute_spam_dist_2d(
            series.get("post_ids", []), spam_map
        )
