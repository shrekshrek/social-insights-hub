...
def run_project_snapshot_pipeline_sync(
    *,
    ...
    try:
            ...
            for future in as_completed([future_entity, future_opinion]):
                try:
                    if future == future_entity:
                        ...
                        logger.info(f"[项目快照] 实体归一完成: snapshot_id={snapshot_id}")
                    else:
                        op_norm_result = future.result()
                        logger.info(f"[项目快照] 观点归一完成: snapshot_id={snapshot_id}")
                except Exception as e:
                    logger.error(f"[项目快照] 归一化任务失败: {e}")

        # 3. 处理实体归一结果
        ent_input_count = ent_norm_result.get("input_count") or len([e for e in top_entities if isinstance(e, dict)])
        ent_program_count = ent_norm_result.get("program_clustered_count") or ent_input_count
        ...
        set_step(stage2, "entity_normalization", "completed", llm_used=bool(ent_norm_result.get("used")))
        ...
        token_usage_entity = ent_norm_result.get("token_stats") or {
            "summary": {"total_calls": 0, "total_input_tokens": 0, "total_output_tokens": 0, "total_tokens": 0, "total_cost_cny": 0.0, "total_duration_seconds": 0.0},
            "call_details": []
        }
        ...
        # 4. 处理观点归一结果
        op_input_count = op_norm_result.get("input_count") or len([t for t in top_topics if isinstance(t, dict)])
        ...
        set_step(stage2, "opinion_normalization", "completed", llm_used=bool(op_norm_result.get("used")) or bool(stage2.get("category_alignment", {}).get("used")))
        ...
        complete_analysis_job_sync(
            ...
            result_data={
                ...
                "llm_used": bool(op_norm_result.get("used")) or bool(stage2.get("category_alignment", {}).get("used")),
            },
            error_message=None,
        )
        snapshot.result_data = result
        ...
        token_usage_summary = (summ.get("llm") or {}).get("token_stats") or {
            "summary": {"total_calls": 0, "total_input_tokens": 0, "total_output_tokens": 0, "total_tokens": 0, "total_cost_cny": 0.0, "total_duration_seconds": 0.0},
            "call_details": []
        }
        complete_analysis_job_sync(
