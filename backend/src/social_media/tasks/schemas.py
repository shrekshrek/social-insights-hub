...
class SocialCommentBase(CustomBaseModel):
    ...
    comment_id_on_platform: str = Field(
        ..., min_length=1, max_length=255, description="平台上的评论ID"
    )
...
class SocialCommentListResponse(CustomBaseModel):
    ...
    @classmethod
    def create(
        cls, items: List[SocialCommentRead], total: int, page: int, page_size: int
    ):
        """创建分页响应"""
        return cls(items=items, total=total, page=page, page_size=page_size)
...
class JSONUploadData(CustomBaseModel):
    ...
    comments: List[RawCommentData] = Field(
        default_factory=list, description="评论列表（支持平台原始格式）"
    )
