...
def _make_state(selected: list, findings: list, questions: list[str] | None = None) -> dict:
    return {
        "query": "乐虎功能饮料市场研究",
...
class TestSynthesizerEmptyBranch:
    ...
    def test_both_empty_triggers_empty_response(self):
        ...
        assert "未找到相关数据" in result["findings_by_question"][
            "乐虎在功能饮料市场的份额是多少?"
        ]["answer_summary"]
    ...
    def test_findings_populated_but_selected_empty_should_NOT_early_return(self):
        ...
        with patch(
            "src.research_agent.nodes.synthesizer.ChatDeepSeek"
        ) as mock_llm_cls:
            mock_llm = MagicMock()
        ...
        assert (
            findings_by_q["乐虎在功能饮料市场的份额是多少?"]["confidence"] == "high"
        )
