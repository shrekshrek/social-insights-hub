"""Tests for spam_distribution_builder module."""

import pytest

from src.social_media.analysis.celery_tasks.aggregation.spam_distribution_builder import (
...
class TestBuildSpamDistributions:
    ...
    def test_ipa_gets_2d_distribution(self):
        ipa = [{"name": "X", "post_ids": [1, 2, 3]}]
        build_spam_distributions(
            aggregated_entities=[],
            aggregated_opinions=[],
            quadrant_data=[],
        ...
        assert ipa[0]["spam_distribution"] == {"high": 2, "low": 1}

    def test_competitor_series_gets_2d_distribution(self):
        series = [{"name": "Brand", "post_ids": [1, 4]}]
        build_spam_distributions(
            aggregated_entities=[],
            aggregated_opinions=[],
            quadrant_data=[],
        ...
        assert series[0]["spam_distribution"] == {"high": 1, "low": 1}
