...
实时/近期新闻监测，与 knowledge_base 区别：时效性优先，不做 RAG，按关键词搜索后聚合分析。

### 数据来源

| 渠道 | 实现方式 | 覆盖 |
|------|---------|------|
| 百度新闻 | Crawl4AI 爬搜索结果页 | 国内主流媒体（核心）|
| DuckDuckGo News | ddgs Python 库，region="cn-zh" | 国际视角 + 部分独立媒体（补充）|

双渠道按 URL 去重后合并，probe 阶段仅用百度，collect 阶段双渠道并发。
