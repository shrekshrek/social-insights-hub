...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">笔记数据</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">查看和管理爬取的笔记内容</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <UCard>
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-gray-600 dark:text-gray-400">总笔记数</p>
            <p class="text-2xl font-bold mt-1">{{ stats.total }}</p>
          </div>
          <div class="p-3 bg-primary-100 dark:bg-primary-900/20 rounded-lg">
            <UIcon name="i-heroicons-document-text" class="text-2xl text-primary-600 dark:text-primary-400" />
          </div>
        </div>
      </UCard>
      ...
      <ClientOnly>
        <template #fallback>
          <div class="text-center py-8">
            <p class="text-gray-600 dark:text-gray-400">加载笔记列表中...</p>
          </div>
        </template>
...
const filteredNotes = computed(() => {
  ...
  if (filters.value.platform) {
    result = result.filter(note => note.platform === filters.value.platform)
  }
...
const paginatedNotes = computed(() => {
  ...
  const items = filteredNotes.value.slice(start, end)
  ...
  return items.map(note => ({
    ...
    title: h('div', { class: 'space-y-1' }, [
      h('a', {
        href: note.note_url || '#',
        target: '_blank',
        class: 'text-sm font-medium text-primary-600 dark:text-primary-400 hover:underline line-clamp-2',
        title: note.title
      }, note.title),
      h('p', {
    ...
    author_name: h('div', { class: 'flex items-center gap-2' }, [
      note.author_avatar
        ...
        : h('div', {
          ...
          h('span', { class: 'text-xs text-gray-600 dark:text-gray-400' }, note.author_name[0])
        ]),
      h('span', { class: 'text-sm' }, note.author_name)
    ]),
    stats: h('div', { class: 'text-xs space-y-1' }, [
      h('div', { class: 'flex items-center gap-3' }, [
        h('span', { class: 'flex items-center gap-1' }, [
          h(resolveComponent('UIcon'), { name: 'i-heroicons-heart', class: 'text-red-500' }),
          h('span', {}, note.like_count?.toLocaleString() || '0')
        ]),
        h('span', { class: 'flex items-center gap-1' }, [
          h(resolveComponent('UIcon'), { name: 'i-heroicons-chat-bubble-left', class: 'text-blue-500' }),
          h('span', {}, note.comment_count?.toLocaleString() || '0')
        ]),
        h('span', { class: 'flex items-center gap-1' }, [
          h(resolveComponent('UIcon'), { name: 'i-heroicons-bookmark', class: 'text-yellow-500' }),
          h('span', {}, note.collect_count?.toLocaleString() || '0')
        ])
      ])
    ]),
    published_at: note.published_at
      ...
      : '-',
    crawled_at: new Date(note.crawled_at).toLocaleString('zh-CN', {
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
      hour: '2-digit',
    ...
    actions: h('div', { class: 'flex items-center gap-1' }, [
      h(resolveComponent('UButton'), {
        ...
        onClick: () => navigateTo(`/crawler-data/notes/${note.id}`)
      }),
      h(resolveComponent('UButton'), {
        color: 'error',
        variant: 'ghost',
        ...
        onClick: async () => {
          ...
          const confirmed = await $confirm(`确定要删除笔记 "${note.title}" 吗？此操作不可恢复。`)
          if (!confirmed) return

          const toast = useToast()
          try {
            await crawlerDataApi.deleteNote(note.id)
            toast.add({
              title: '删除成功',
              description: `笔记 "${note.title}" 已被删除`,
              color: 'success'
            })
            await loadNotes()
          } catch (error) {
...
const loadNotes = async () => {
  loading.value = true
  try {
    const result = await crawlerDataApi.getNotes({
      skip: 0,
    ...
    stats.value.byPlatform = result.items.reduce((acc, note) => {
      acc[note.platform] = (acc[note.platform] || 0) + 1
      return acc
...
// 监听 filteredNotes 变化更新 total
watch(filteredNotes, (newValue) => {
